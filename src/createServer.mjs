...
const typeDefs = `#graphql
  ...
  type Query {
    ...
    recentWorkComments(limit: Int = 3): [HomeComment!]!
...
const resolvers = {
  Query: {
    ...
    recentWorkComments: async (_, { limit }, { repo }) => repo.listRecentWorkComments({ limit }),
    ...
    unreadDirectMessagesCount: async (_, __, { repo, currentUser }) => repo.countUnreadPrivateMessages({ userId: requireAuth(currentUser).id }),
  ...
  Mutation: {
    ...
    adminCreateWorkGenre: async (_, { input }, { currentUser, repo, adminUserIds }) => {
      const user = requireAuth(currentUser);
      if (!isAdminUser(user, adminUserIds)) {
        throw new GraphQLError('Только администратор может управлять жанрами', { extensions: { code: 'FORBIDDEN' } });
      }
      const name = String(input.name ?? '').trim();
      if (!name) {
        throw new GraphQLError('Название жанра обязательно', { extensions: { code: 'BAD_USER_INPUT' } });
      }
      return repo.createWorkGenre({ sectionCode: input.sectionCode, name, sortOrder: input.sortOrder });
    },
    adminUpdateWorkGenre: async (_, { genreId, input }, { currentUser, repo, adminUserIds }) => {
      ...
      return repo.updateWorkGenre({ genreId, name, sortOrder: input.sortOrder });
    },
  ...
  Author: {
    ...
    canReceivePrivateMessages: (parent) => !Boolean(parent?.isClassic || parent?.isMemorialPage),
