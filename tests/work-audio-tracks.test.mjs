...
test('createWork and updateWork expose and forward multiple audio tracks', async () => {
  const calls = [];
  const tracks = [
    { url: 'https://cdn.test/audio/part-1.mp3', fileName: 'part-1.mp3' },
    { url: 'https://cdn.test/audio/part-2.mp3', fileName: 'part-2.mp3' },
  ];
  const repo = {
  ...
  try {
    ...
    const create = await server.executeOperation({
      query: `mutation Create($input: CreateWorkInput!) { createWork(input: $input) { audioTracks { url fileName } } }`,
    ...
    const update = await server.executeOperation({
      query: `mutation Update($input: UpdateWorkInput!) { updateWork(workId: "100", input: $input) { audioTracks { url fileName } } }`,
      variables: { input: { sectionCode: 'poetry', title: 'Audio work', audioTracks: tracks } },
    }, { contextValue });
...
test('repository maps legacy audio columns to a fallback audio track', async () => {
  ...
  assert.deepEqual(work.audioTracks, [{ url: 'https://cdn.test/audio/legacy.mp3', fileName: 'legacy.mp3' }]);
});

test('repository persists audio tracks as JSONB on create and update', async () => {
  const calls = [];
  const tracks = [{ url: 'https://cdn.test/audio/part-1.mp3', fileName: 'part-1.mp3' }];
  ...
  repo.getWorkById = async () => workWithDefaults({ audioTracks: tracks });
